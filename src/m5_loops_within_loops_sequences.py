...
# Test 2:
    expected = [-200, 5, 6, -3, 29, 70, 43]
    answer = integers([(-200, 5, 6),
                       '',
                       [-3],
                       'hello',
                       [],
...
# Test 2:
    expected = 'HELLO'
    answer = big_letters([['H'], 'HELLO', (3, 44, 6, 'hi'), 0])
    print('Expected is:', expected)
    print('Actual is:  ', answer)
...
def big_letters(sequence_of_sequences):
    ...
    for k in range(len(sequence_of_sequences)):
        if type(sequence_of_sequences[k]) != str:
            continue  # it will move to the next k value
        for j in range(len(sequence_of_sequences[k])):
            if sequence_of_sequences[k][j].isupper():
                new_str = new_str + sequence_of_sequences[k][j]
    return new_str
